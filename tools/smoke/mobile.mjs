...
try {
  ...
  layout.quality = await page.evaluate(async () => (await import('/src/city/Quality.ts')).QUALITY_LEVEL);
  layout.colliders = await page.evaluate(() => window.__xarita.city.physics.world.colliders.len());
  ...
  assert.ok(layout.pixelRatio <= 1.25, 'phone render resolution must be capped');
  ...
  assert.equal(layout.quality, 'low', 'phones start in the light graphics profile');
  assert.ok(layout.colliders < 200, `static landmark colliders must be baked (${layout.colliders})`);
